//https://search.cplusplus.me/js/ajax.js
...
function get_pagerank() {
    ...
    if (domain) {
        $("#pagerank").ajaxStart(function() {
            $(this).html('<div class="loading">正在加载...</div>')
        }) $.ajax({
            type: "post",
            cache: false,
            url: action['pagerank'],
...
function checkhomepage() {
    if (document.getElementById('logo').isHomePage("http://tool.114la.com/index.html") || document.getElementById('logo').isHomePage("http://tool.114la.com") || document.getElementById('logo').isHomePage("http://tool.114la.com/")) {
        return true;
    } else {
        return false;
    }
